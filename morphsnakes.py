...
class MorphACWE(object):
    ...
    def step(self):
        ...
        inside = u>0
        outside = u<=0
        c0 = int(int(data[outside].sum()) / int(outside.sum()))
        c1 = int(int(data[inside].sum()) / int(inside.sum()))
        #c0=float(c0)
        #c1=float(c1)
        #print(c0)
        #print(c1)
        ...
        #resaux = np.copy(abs_dres) #copy(abs_dres)
        resaux=np.copy(abs_dres)
        #resaux[abs_dres < 0] = 0
        resaux[abs_dres > 0] = 1
        aux = np.int64(np.int64(resaux) * (self.lambda1*(np.int64(data) - c1)*(np.int64(data) - c1) - self.lambda2*(np.int64(data) - c0)*(np.int64(data) - c0)))
